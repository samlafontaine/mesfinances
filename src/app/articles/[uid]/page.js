...
function LatestArticle({ article }) {
  ...
  return (
    <li>
      ...
      <p className="font-sans tracking-tighter text-slate-500">
...
export default async function Page({ params }) {
  ...
  return (
    ...
    >
      <Bounded>
        <Link href="/" className="font-sans tracking-tight text-slate-500">
          &larr; Voir les autres articles
        </Link>
      </Bounded>
      <article>
        <Bounded className="pb-0">
          <h1 className="mb-3 text-3xl font-sans font-semibold tracking-tighter text-slate-800 md:text-4xl">
            <PrismicText field={article.data.title} />
          </h1>
          <p className="font-sans tracking-tighter text-slate-500">
            {dateFormatter.format(date)}
